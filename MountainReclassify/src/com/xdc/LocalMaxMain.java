...
public static class MyReducer extends Reducer<LongWritable,BytesWritable,LongWritable,BytesWritable> { 
    ...
    public void reduce(LongWritable key, Iterable<BytesWritable> values, 
        Context context) throws IOException, InterruptedException {				
			...
			int data;
			byte[] maxbytes = new byte[length-4];//减4的原因是去掉前4个字节，因为每个瓦片的前四个字节表示的是数据类型（0或1），本身是不属于瓦片的
			for (int i = 0; i < length-4; i += 4) {
				data = Float.floatToIntBits(max[(i / 4) +1]);
  ...
  public static void main(String[] args) throws Exception {
			    ...
			    */    
				 if(args.length !=6){
					 System.out.println("Usage: [inputPath1] [inputPath2] [outputPath] [reduceTaskNum] [mapCompress][reduceCompress]");
					 return;
				 }	  
				 ...
				 Job job =Job.getInstance(conf, "LocalMax_"+temp[temp.length-1]);
